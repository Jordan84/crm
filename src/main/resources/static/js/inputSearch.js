		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.personList.length == 0 || data.personList.length == null) {
					...
					// $(newInputDiv).blur(function(){
					// 	$(newInputDiv).val(subText((data.personList)[0].loginName));
					// 	$(newHiddentInput).val((data.personList)[0].id);
					// 	succallback();
					// 	$(".searchDiv").css("display", "none");
					// 	// $("zhugePhone").val((data.listPerson)[0].publicPhone);//诸葛io拿到phone
					// 	$(".searchDiv ul").empty();
					// });
				} else {
				...
				$.each(data.personList, function(index, item) {
				    console.info("personId"+item.id)
                    htmlstr += "<li data-loginName='"+item.loginName+"' value='" + item.id + "'>" + item.loginName + " | "+item.cnName+" | "+item.phone+"</li>";
				});
                $(".searchDiv ul").html(htmlstr);
				...
				$(".searchDiv ul li").click(function() {
					...
					console.info("lallalaalal"+$(newHiddentInput).attr("value"));
